...
class CIFAR10p1(Dataset):
    ...
    def __init__(self, root, split='train', version='v6', transform=None, download=False):
        super().__init__()
        if download:
            #work in progress
            os.system(f'mkdir {root}')
            file_link = 'https://github.com/modestyachts/CIFAR-10.1/blob/master/datasets/cifar10.1_v6_data.npy'
            os.system(f'wget {file_link} -P {root}')
